...
class StartNode extends HAMINode<Shared, Config> {
    kind(): string {
        return 'tbc-dex:discover-records-flow-start:nx';
    }

    async post(shared: Shared): Promise<string> {
        assert(shared.system?.rootDirectory, 'shared.system.rootDirectory is required for DiscoverRecordsFlowNx');
        assert(shared.system?.protocol, 'protocol must be resolved before discover-records-flow:nx');
...
export class DiscoverRecordsFlowNx extends HAMIFlow<Shared, Config> {
    startNode: Node;
    ...
    kind(): string {
        return 'tbc-dex:discover-records-flow:nx';
    }
    ...
    async prep(shared: Shared): Promise<void> {
        ...
        fsSequence
            ...
            .next(this.config.verbose ? n('tbc-system:add-manifest-messages', {
                source: 'discover-records-flow:nx',
                level: 'debug',
            }) : new Node())
            // 1. Setup Query
            .next(n('core:mutate', {
