...
export class StoreRecordsNewNode extends HAMINode<TBCRecordFSStorage> {
    constructor(maxRetries?: number, wait?: number) {
        super(maxRetries, wait);
    }

    // TODO - Rename to `tbc-record-fs:store-records`
    //   Once `tbc-record:store-records` have been replaced at call sites
    //   of `tbc-record-fs:store-records`, then the old one needs to be
    //   cleaned up and the `-new` dropped.  The class name should
    //   also be updated.
    kind(): string {
        return "tbc-record-fs:store-records-new";
    }
