...
## 6. Skills System (sys/skills)

### 6.1. Definition
A Skill is a "Specification for Execution"—the instructional "How-To" that satisfies a Motivation.

### 6.2. Structure
Located in `sys/skills/{skill-name}/`. 
* `definition.md`: Instructional system prompt/manual for the agent.
* `tools/` (Optional): Implementation-specific scripts or JSON (e.g., MCP definitions) for interface consumption.
