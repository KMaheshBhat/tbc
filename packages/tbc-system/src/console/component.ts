...
import { MessageLevel, MessageNode } from "../message/component";

const consoleFlowKind = 'tbc:flow:console'
const consoleIntentKind = 'tbc:intent:console:flush';
...
function isConsoleIntent(intent: Intent): intent is ConsoleIntent {
...
// TODO: KABOOM!?
export function WithConsoleComponent<TBase extends GenericConstructor<Payload>>(
  Base: TBase
): TBase & GenericConstructor<ConsoleComponent> {
  return class extends Base {
    private flowId: string | undefined
    private messageRootNodeId: string | undefined
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    constructor(...args: any[]) {
      super(...args);
      this.flowId = undefined;
      this.messageRootNodeId = undefined;
    }
    ...
    consoleFlow(
      ...
      if (!this.messageRootNodeId) {
        throw new Error(
          `${consoleFlowKind}:${this.flowId}: messageRootNodeId is required`
        );
      }
      const kind = consoleFlowKind;
      return new ConsoleFlow(options);
    }
  }
}
