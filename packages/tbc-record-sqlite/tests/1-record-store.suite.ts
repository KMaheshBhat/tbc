...
describe('SQLiteStore Contract (RecordStore)', () => {
    ...
    describe('Metadata & Safety', () => {
        it('updates updated_at on store() re-run', async () => {
            const id = 'meta_1';
            const COL = 'meta';

            await store.store(COL, [{ id, kind: 'k', data: { v: 1 } }]);
            
            const db = (store as any).db;
            const getUpdate = () => db.query('SELECT updated_at FROM record WHERE record_id = ?').get(id).updated_at;
            
            const time1 = getUpdate();
            await new Promise(r => setTimeout(r, 10));
            
            await store.store(COL, [{ id, kind: 'k', data: { v: 2 } }]);
            const time2 = getUpdate();
