...
export class SchemaWatermarkNode extends HAMINode<TBCViewStorage> {
    constructor(maxRetries?: number, wait?: number) {
        super(maxRetries, wait);
    }

    kind(): string {
        return "tbc-view:schema-watermark";
    }

    async prep(shared: TBCViewStorage): Promise<SchemaWatermarkNodeInput> {
    ...
    async exec(params: SchemaWatermarkNodeInput): Promise<SchemaWatermarkNodeOutput> {
        ...
        for (const record of params.processedRecords) {
            ...
            params.viewStore.setWatermark(record.node.id, 'schema', schema.status, schema.message, 'schema-watermark');
    ...
    async post(shared: TBCViewStorage, _prepRes: SchemaWatermarkNodeInput, execRes: SchemaWatermarkNodeOutput): Promise<string | undefined> {
