...
export class MetadataExtractorNode extends HAMINode<TBCViewStorage> {
    constructor(maxRetries?: number, wait?: number) {
        super(maxRetries, wait);
    }

    kind(): string {
        return "tbc-view:metadata-extractor";
    }

    async prep(shared: TBCViewStorage): Promise<MetadataExtractorInput> {
    ...
    async post(shared: TBCViewStorage, _prepRes: MetadataExtractorInput, execRes: MetadataExtractorOutput): Promise<string | undefined> {
        // Upsert nodes, attributes, and edges to database
        shared.viewStore!.beginTransaction();

        try {
            for (const record of execRes.processedRecords) {
                shared.viewStore!.upsertNode(record.node);
                shared.viewStore!.upsertAttributes(record.node.id, record.attributes);
                shared.viewStore!.upsertEdges(record.node.id, record.edges);
            }

            shared.viewStore!.commit();
        } catch (error) {
            shared.viewStore!.rollback();
            throw error;
        }
