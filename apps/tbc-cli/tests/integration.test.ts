...
describe("TBC-CLI Integration", () => {
    ...
    test("🐵 LETS-GO: running sys init with companion and prime flags is successful", async () => {
        ...
        expectUUID(primeId);
        expect(output).toContain("Generated companion.id record");
        expect(output).toContain("Generated prime.id record");
        expect(output).toContain("This appears to be a valid TBC root directory.");
    });
